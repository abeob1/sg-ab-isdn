...
Namespace AE_ISDN_A02
    ...
    Module modCommon
        ...
        Public Function ExportToHTML(ByVal sCardCodeFrom As String, _
                        ByVal sCardCodeTo As String, _
                        ByVal _Date As Date, _
                        ByVal _Condtion As String, _
                        ByVal sTargetFileName As String, _
            ...
            Dim sCompanyName As String = String.Empty
            Dim sSVCID As String = String.Empty
            CrFormatTypeOptions.HTMLBaseFolderName = sTargetFileName
            CrFormatTypeOptions.HTMLFileName = "StatementofAccount_" & Format(Now.Date, "dd-MM-yyyy") & ".HTML"

            '' Dim jj = "StatementofAccount_" & Format(Now.Date, "dd-MM-yyyy") & ".HTML"
            Try
                sFuncName = "ExportToPDF()"
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Starting Function", sFuncName)
                ...
                End With
                'Export the report 
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Before Export HTML", sFuncName)
                objReport.Export()
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("After Export HTML", sFuncName)

                ExportToHTML = RTN_SUCCESS
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with SUCCESS", sFuncName)
            Catch ex As Exception
                sErrDesc = ex.Message
                ExportToHTML = RTN_ERROR
                Call WriteToLogFile(sErrDesc, sFuncName)
                Throw New ArgumentException(sErrDesc)
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed function with ERROR", sFuncName)
            Finally
        ...
        Public Function SendEmailNotification(ByVal sfileName As String, ByVal sSenderEmail As String, ByVal sRemarks As String, ByRef sErrDesc As String) As Long
        ...
        Public Function SendEmailNotificationHTML(ByVal sfileName As String, ByVal sSenderEmail As String, ByVal sRemarks As String, ByRef sErrDesc As String) As Long
            ...
            Try
                sFuncName = "SendEmailNotificationHTML()"
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Starting Function...", sFuncName)
                Console.WriteLine("Starting Function... " & sFuncName)
                sfileName = sfileName & "\Statement_of_Account_V2\StatementofAccount_" & Format(Now.Date, "dd-MM-yyyy") & ".HTML"
                p_SyncDateTime = Format(Now, "dddd") & ", " & Format(Now, "MMM") & " " & Format(Now, "dd") & ", " & Format(Now, "yyyy") & " " & Format(Now, "HH:mm:ss")
                '--------- Message Content in HTML tags
                Dim sBody As String = String.Empty
                ...
                'Using reader As StreamReader = File.OpenText(sfileName)
                '    sBody = sBody & reader.ReadToEnd()
                'End Using
                ...
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with SUCCESS", sFuncName)
                SendEmailNotificationHTML = RTN_SUCCESS
            Catch ex As Exception
                sErrDesc = ex.Message
                oMail.Dispose()
                WriteToLogFile(ex.Message, sFuncName)
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with ERROR", sFuncName)
                Console.WriteLine("Completed with Error " & sFuncName)
                SendEmailNotificationHTML = RTN_ERROR
            Finally
                oMail.Dispose()
                File.Delete(sfileName)
            End Try
        ...
        Public Function TransactionLog(ByVal sBpCode As String, ByVal sBpName As String, ByVal dDate As Date, ByVal dBalance As Double, ByVal sEmailAdd As String, _
            ...
            Try
                ...
                oRset.DoQuery("select (max(CAST(IFNULL(T0.""Code"",0) As Integer))+1) ""Code"" FROM ""@AE_ESOALOG""  T0")
                ...
                oUserTable.UserFields.Fields.Item("U_Balance").Value = dBalance.ToString
        ...
        'End Sub
        Function SOAValidation(FormUID As SAPbouiCOM.Form, ByRef sErrDesc As String, ByVal sboApplication As SAPbouiCOM.Application) As Long
            Dim sFuncName As String = String.Empty
            'Dim sboApplication As ClsMain
            'sboApplication = New ClsMain

            Dim oItemMethod As SAPbouiCOM.Item = FormUID.Items.Item("Item_15")
            Dim oComboMethod As SAPbouiCOM.ComboBox
            oComboMethod = oItemMethod.Specific
            Dim oItemFormat As SAPbouiCOM.Item = FormUID.Items.Item("Item_18")
            Dim oComboFormat As SAPbouiCOM.ComboBox
            oComboFormat = oItemFormat.Specific
            Try
                sFuncName = "SOAValidation()"
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Starting function", sFuncName)
                If oComboMethod.Value = "" Then
                    sboApplication.StatusBar.SetText("SOA Method is  Missing ....... !", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Warning)
                ElseIf oComboFormat.Value = "" Then
                    sboApplication.StatusBar.SetText("SOA Format is  Missing ....... !", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Warning)
                    'FormUID.ActiveItem = "Item_18"
                End If
                SOAValidation = RTN_SUCCESS
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with SUCCESS", sFuncName)
            Catch ex As Exception
                sboApplication.StatusBar.SetText("SOAValidation Function : " & ex.Message, SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                sErrDesc = ex.Message
                Call WriteToLogFile(sErrDesc, sFuncName)
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with ERROR", sFuncName)
                SOAValidation = RTN_ERROR
            End Try
            Return RTN_SUCCESS
        End Function
        Function HeaderValidation(FormUID As SAPbouiCOM.Form, ByRef sErrDesc As String, ByVal sboApplication As SAPbouiCOM.Application) As Long
            Dim sFuncName As String = String.Empty
            ''Dim sboApplication As ClsMain
            ''sboApplication = New ClsMain
            Try
                ...
                If FormUID.Items.Item("BPFrom").Specific.string = String.Empty Then
                    sboApplication.StatusBar.SetText("BP From is Missing ....... !", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Warning)
                    FormUID.ActiveItem = "BPFrom"
                    Return RTN_ERROR
                ElseIf FormUID.Items.Item("BPTo").Specific.string = String.Empty Then
                    sboApplication.StatusBar.SetText("BP To is Missing ....... !", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Warning)
                    FormUID.ActiveItem = "BPTo"
                    Return RTN_ERROR
                ElseIf FormUID.Items.Item("Item_7").Specific.string = String.Empty Then
                    sboApplication.StatusBar.SetText("Aging Date is Missing ....... !", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Warning)
            ...
            Catch ex As Exception
                sboApplication.StatusBar.SetText("HeadValidation Function : " & ex.Message, SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                sErrDesc = ex.Message
                Call WriteToLogFile(sErrDesc, sFuncName)
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with ERROR", sFuncName)
        ...
        Function RowValidation(FormUID As SAPbouiCOM.Form, ByRef oApplication As SAPbouiCOM.Application, ByRef sErrDesc As String, ByVal sboApplication As SAPbouiCOM.Application) As Long
            Dim sFuncName As String = String.Empty
            ...
            Try
                ...
                For mjs As Integer = 1 To oMatrix.RowCount
                    ...
                    If oCheck.Checked Then
                        If String.IsNullOrEmpty(oMatrix.Columns.Item("Col_4").Cells.Item(mjs).Specific.String) Then
                            ...
                            oRowCount += 1
                        End If
                        '' '' ''Commented for testing - 09Dec2016 (Need to uncomment)
                ...
                If bErrorFlag = True Then
                    ...
                    sboApplication.StatusBar.SetText("Validation Error!  Some BP`s not having Email Id`s / The Balance is Zero ! ", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                    Return RTN_ERROR
                End If
            ...
            Catch ex As Exception
                sboApplication.StatusBar.SetText("RowValidation Function : " & ex.Message, SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                sErrDesc = ex.Message
                Call WriteToLogFile(sErrDesc, sFuncName)
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with ERROR", sFuncName)
        ...
        Function Loading_AgingDetails(ByRef FormUID As SAPbouiCOM.Form, ByRef oApplication As SAPbouiCOM.Application _
            ...
            Try
                ...
                'CALL "AE_SP002_SOA" ('AAA','C20000','2016-11-30')
                sQry = "CALL AE_SP001_SOA ('" & FormUID.Items.Item("BPFrom").Specific.String & "','" & FormUID.Items.Item("BPTo").Specific.String & "','" & AgingDate & "')"
                ...
                FormUID.Items.Item("Item_8").Specific.columns.item("Col_7").databind.bind("OCRD", "SOAMethod")
            ...
            Catch ex As Exception
                oApplication.StatusBar.SetText("HeadValidation Function : " & ex.Message, SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                sErrDesc = ex.Message
                Call WriteToLogFile(sErrDesc, sFuncName)
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with ERROR", sFuncName)
        ...
        Public Function Print_Report(ByVal sCardCodeFrom As String, _
                              ByVal sCardCodeTo As String, _
                              ByVal _Date As Date, _
                              ByVal sReportFileName As String, _
                              ByRef oApplication As SAPbouiCOM.Application, _
            ...
            Try
                ...
                cryRpt.Load(sRptFileName, CrystalDecisions.[Shared].OpenReportMethod.OpenReportByTempCopy)
                ...
                cryRpt.SetParameterValue(0, sCardCodeFrom)
                cryRpt.SetParameterValue(1, sCardCodeTo)
                cryRpt.SetParameterValue(2, _Date)
                ...
                ' if the object is a subreport, then apply logon info on each table of that sub report
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Checking for subreport", sFuncName)
                For iCount = 0 To cryRpt.ReportDefinition.Sections.Count - 1
                    For intCounter = 0 To cryRpt.ReportDefinition.Sections(iCount).ReportObjects.Count - 1
                        With cryRpt.ReportDefinition.Sections(iCount)
                            If .ReportObjects(intCounter).Kind = CrystalDecisions.Shared.ReportObjectKind.SubreportObject Then
                                mySubReportObject = CType(.ReportObjects(intCounter), CrystalDecisions.CrystalReports.Engine.SubreportObject)
                                mySubRepDoc = mySubReportObject.OpenSubreport(mySubReportObject.SubreportName)
                                'get the subreport parameter count to exclude passing data
                ...
                Next
                'Check if there are parameters or not in report and exclude the subreport parameters.
                intCounter = cryRpt.DataDefinition.ParameterFields.Count - iSubRParaCount
                'As parameter fields collection also picks the selection formula which is not the parameter
                ' so if total parameter count is 1 then we check whether its a parameter or selection formula.
                If intCounter = 1 Then
                ...
                'End If
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Selecting Default Printer", sFuncName)
                cryRpt.PrintToPrinter(1, True, 1, 100)
                'System.Threading.Thread.Sleep(50)
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Printing SOA", sFuncName)
                Print_Report = RTN_SUCCESS
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with SUCCESS", sFuncName)
            ...
            Catch ex As Exception
                oApplication.StatusBar.StatusBar.SetText(ex.Message, SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Warning)
                sErrDesc = ex.Message
                Print_Report = RTN_ERROR
                Call WriteToLogFile(sErrDesc, sFuncName)
                Throw New ArgumentException(sErrDesc)
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed function with ERROR", sFuncName)
            Finally

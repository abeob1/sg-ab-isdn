...
Namespace AE_ISDN_A02
    Public Class ClsMain
        ...
        Sub SBO_Application_MenuEvent(ByRef pVal As SAPbouiCOM.MenuEvent, ByRef BubbleEvent As Boolean) Handles SBO_Application.MenuEvent
            ...
            Try
                If (pVal.BeforeAction And pVal.MenuUID = "FSOA") Then
                    If p_iSOA = 0 Then
                        Dim activeForm As SOA
                        Dim sErrDesc As String = String.Empty
                        Dim date1 As Date = DateTime.Now
                        'MsgBox(date1.ToString("dd.MM.yyyy"))
                        activeForm = New SOA(ClsFunction.Company, Application.SBO_Application, sErrDesc)
                        ' If activeForm.Alive = False Then
                        If String.IsNullOrEmpty(sErrDesc) Then
                            Dim oForm As SAPbouiCOM.Form = SBO_Application.Forms.Item("SOA")
                            Set_Conditions(oForm, "")
                            oForm.Items.Item("Item_5").Specific.string = date1.ToString("dd.MM.yyyy")
                            activeForm.Show()
                        End If

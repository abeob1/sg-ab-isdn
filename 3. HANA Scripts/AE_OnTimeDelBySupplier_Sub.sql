Create PROCEDURE "AE_OnTimeDelBySupplier_Sub"
(
 IN DateFrom DATE,
 IN DateTo DATE,
...
T0."CardCode",T0."CardName",count(T0."DocNum") as GRN_No
--,sum(case when DAYS_BETWEEN (T3."ShipDate",IFNULL(T3."U_SupplierETD",T0."DocDueDate") ) <=0 then 1
...
-- sum(case when DAYS_BETWEEN (T3."ShipDate",IFNULL(T3."U_SupplierETD",T0."DocDueDate") ) <=0 then 100
-- else 0 end)/count(T0."DocNum") As Performance
-- ,rank() OVER (PARTITION BY T0."CardCode" ORDER BY 
-- sum(case when DAYS_BETWEEN (T3."ShipDate",IFNULL(T3."U_SupplierETD",T0."DocDueDate") ) <=0 then 100
-- else 0 end)/count(T0."DocNum") DESC ) AS Rank
 
 ,sum(case when DAYS_BETWEEN (IFNULL(T8."ShipDate",T5."DocDueDate"),T0."DocDate" ) <=0 then 1
 else 0 end) as CountOnTime
